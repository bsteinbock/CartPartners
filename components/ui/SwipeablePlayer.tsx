...
const SwipeablePlayerItem = ({ player }: { player: Player }) => {
  const router = useRouter();
  const { deletePlayer, updatePlayer, currentLeagueId, leagues } = useDbStore();
    ...
    (player: Player) => {
      Alert.alert(
        ...
        `Are you sure you want to delete ${player.name}? This action cannot be undone. It
         will remove the player from all rounds and groups in the league ${league?.name}.`,
